...
**Your GPU allocation (match the card to the task — don't burn A100 hours on cheap work):**

You have three tiers available, and the smart play is using each for what it's best at. Colab Pro bills compute units by the hour and the A100 is the priciest tier, so reserving it for actual training keeps your quota alive long enough to iterate across all three phases.

- **A100 (40GB) — training runs only.** This is where speed matters: large batches, your 4096-token trajectories without OOM, and Phase 2B's GKD (which generates *during* training and is genuinely slow on weaker cards). When you're iterating on hyperparameters — 1 vs 2 epochs, different ranks, different data subsets — the fast turnaround here is the iteration speed you want. A 7B QLoRA run that's a slog elsewhere is a few minutes on an A100. 40GB is plenty for a 7B QLoRA; no need to hold out for the 80GB.
- **L4 (24GB) — serving, rollouts, and eval.** Serving the student with vLLM for evaluation, generating rejection-sampling rollouts (Phase 2A), watching episodes run. These are *throughput* tasks, not *training* tasks — a 7B in 4-bit runs fine here, at far fewer compute units per hour than the A100. This is your default working card.
- **T4 (free tier) — plumbing.** Writing and testing the data scripts, the comparator, the agent loop logic — anything that barely touches the GPU. No reason to spend Pro units debugging Python.

> **You don't always get the card you ask for.** Colab allocates by availability; the A100 may not be offered during high demand (it's more reliable on Pro+). Set your preferred GPU in the runtime-type menu, but make the GPU-check cell above a habit at the top of *every* notebook, and have your code adapt `per_device_train_batch_size` to the card it actually landed on rather than assuming. More VRAM here buys you *iteration speed and the ability to run Phase 2B at all* — not a fundamentally better model. The recipe (7B + QLoRA) doesn't change; only how fast you can turn the crank does.
...
#### The false-positive problem (read this twice — it's the subtlest failure in the project)
 
Execution match asks "did the predicted query return the same table as the gold query?" But the question you *actually* care about is "is the predicted query *semantically correct*?" Those are not the same question, and the gap between them is where false positives live: a query that is **wrong in general** but returns the **right table on this particular database** gets marked correct. Your comparator is a *proxy* for correctness, and a proxy is exactly what optimization pressure learns to exploit (this is Goodhart's law, and it's worth understanding as a general ML concept, not just a SQL nuisance).
 
**How false positives actually arise — the taxonomy.** It's not just "both queries happen to match." There are distinct, recurring mechanisms:
 
- **Empty-result collisions.** The gold query legitimately returns zero rows for this question, *and* the predicted query returns zero rows for an unrelated wrong reason (a typo'd filter, a bad join that eliminates everything). Two different bugs producing the same empty table. This is the single most common false positive.
- **Single-value coincidences.** Aggregates (`COUNT`, `SUM`, `AVG`) collapse to one number. A wrong query can land on the same number — especially `COUNT` results like 0, 1, or small integers that recur across many questions.
- **Data-distribution accidents.** The query is structurally wrong but the *specific data in this one database* hides it: a missing join condition that doesn't change the result because the data happens to have a 1:1 relationship; a filter on the wrong column whose values happen to be identical to the right column's; a missing `DISTINCT` on data that has no duplicates anyway. The query would break on different data — but you only test it on one instance.
- **Ordering / limit coincidences.** `ORDER BY` on the wrong column that happens to produce the same order; `LIMIT 1` that returns the right row by luck.
**Why it bites differently in each phase — and why Phase 2 is where it's lethal:**
 
- **In eval (Phase 0):** false positives *inflate your reported accuracy*. You conclude the model is better than it is. Bad, but it's a passive measurement bias — it doesn't make the model worse.
- **In off-policy filtering (Phase 1):** a false-positive *teacher* trajectory slips into training data, teaching the student a wrong-but-plausible pattern. This is noise — annoying, bounded.
- **In on-policy training (Phase 2A/2B):** this is the lethal case. The student generates a wrong query, your reward marks it "correct," you retrain the student on its *own* mistake, and the next round of rollouts inherits and amplifies it. Because on-policy training applies active optimization pressure *toward whatever your reward rewards*, it will actively *seek out* the false positives — the model discovers that "emit a query that returns an empty set" passes surprisingly often and learns to do it. That's **reward hacking**, and it's a self-reinforcing loop, not static noise. This is the concrete reason the guide keeps warning that the comparator must be hardened before Phase 2.
**How to navigate it — layered defenses, ranked by leverage:**
 
1. **Test-suite accuracy — the principled fix, use it for anything that matters.** Spider ships a test-suite evaluation that runs each query against *many* database instances per schema, augmented specifically so that any two semantically *different* queries will disagree on at least one instance. A query that only coincidentally matched on the original DB gets caught the moment it's run against a perturbed instance. A prediction must match the gold on *all* instances to count as correct, which collapses the false-positive rate to near zero — it's a tight approximation of true semantic equivalence. **Use this for your reported headline numbers, and for the on-policy correctness filter where false positives are most dangerous.**
2. **Degenerate-match guards — cheap, always on for training data.** When deciding whether a trajectory is "correct" enough to *train on*, reject the categories most prone to coincidence: empty result sets, single-scalar results you're not confident about, and trivial queries (bare `SELECT *`, queries that ignore the question's constraints). A correct-but-trivial match is worth little as a training example and risks being a false positive — discard it. (You can still *count* these in eval if you want a complete accuracy number, but don't *learn* from them.)
3. **Harden the comparator's own normalization — so it doesn't manufacture errors.** Your `normalize()` is itself a source of both false positives and false negatives if it's sloppy. If you sort rows when the gold query had `ORDER BY`, you'll mark a wrong-order query correct (false positive). If you don't handle types/duplicates consistently, you'll mark a *right* query wrong (false negative). The order-sensitivity branch in the code above exists precisely for this; get it right and test it on hand-built cases.
4. **Monitor for reward hacking during on-policy rounds — your early-warning system.** Track the *distribution* of result types among trajectories your filter marks "correct." If, over rounds, you see a rising share of empty-set or single-value passes, that's the model finding the exploit. Catching this early (and tightening the filter or switching to test-suite scoring) saves you from training several rounds on garbage.
**The asymmetry worth understanding.** False positives (wrong marked right) inflate accuracy and poison training; false negatives (a *valid alternative* query marked wrong because it phrased the SQL differently) deflate accuracy and waste good training data. Execution-based eval exists *because* it tolerates valid alternatives — that's its whole advantage over rigid exact-string-match, which is riddled with false negatives. But that same permissiveness is exactly what opens the false-positive door. Test-suite accuracy is the sweet spot: it keeps the false-negative tolerance (alternative correct queries still pass) while slamming the false-positive rate down (coincidental matches fail on some instance). That's why it's the recommended scoring path rather than retreating to exact match.
 
**The practical two-tier strategy** (what to actually do): keep your fast in-process `execution_match` as the default for *iteration and quick checks* — it's instant and good enough to see if you're in the right ballpark. Use the official Spider **test-suite** evaluation for (1) every number you *report* and (2) the correctness filter that builds *on-policy* training data. Fast-and-loose for iterating, rigorous-and-slow for the two places a false positive does real damage.
 
> **Task for Claude Code:** *"Integrate the official Spider test-suite evaluation script as an alternative scoring path in `src/eval.py`, behind a `--official` flag, while keeping the fast in-process `execution_match` as the default for iteration. Also add a `--reject-degenerate` flag to `filter.py` that drops empty-result and trivial (`SELECT *`, no-WHERE-when-question-implies-one) matches when building training data, and have it log a count of how many were dropped."*
...
Serve the **untrained** student (`Qwen/Qwen2.5-Coder-7B-Instruct`) with vLLM **on the L4** (serving is a throughput task — save the A100 for training), point the agent loop at it, run over Spider `dev.json`, and compute execution accuracy with the official script. Append the result to `results/eval_runs.jsonl` with a label like `{"model": "qwen2.5-coder-7b-instruct", "phase": "baseline", "ex": 0.xx}`.
...
Upload `train.jsonl` (git push or Drive), open `01_sft_unsloth.ipynb` **on the A100** — this is a training run, so it's where the A100 earns its keep (large batch, full 4096-token sequences, fast turnaround for hyperparameter iteration). The core:
...
> This phase straddles two cards: the rollout-generation steps are *serving* work (L4), the retraining step is a *training* run (A100). If switching runtimes mid-phase is annoying, generate and save `selfgen_train.jsonl` to Drive on the L4 session, then open a fresh A100 session just for the SFT cell.
...
**Requirements:** the teacher must be open-weights (you need its logits), so run a 14–32B coder model on Colab as the teacher. This is more memory and more complexity — only attempt it once 2A works. **Run this on the A100, and watch VRAM carefully:** you're holding *both* a 7B student and a 14B+ teacher in memory at once, plus generating during training. A 7B (4-bit) student + 14B teacher is feasible on the 40GB A100; a 32B teacher likely is not without offloading. Start with the 14B teacher — if even that is tight, this is the place a larger A100 (80GB, more reliable on Pro+) would actually help, but don't reach for it until the 14B setup proves too small.
...
- GKD **generates during training** (for the on-policy fraction), so it's slow and memory-hungry even on the A100. Keep sequences short, batch small, and expect to babysit OOM.
...
6. **Capacity ceiling at tiny sizes.** Below ~7B the agent loops and scores near zero; you'll wrongly blame distillation. With your A100 there's no memory reason to drop below 7B — stay at 7B. (§1.2)
...
11. **Burning A100 quota on cheap work.** Serving, rollouts, eval, and plumbing don't need the A100 — running it for those torches your compute units and leaves you unable to iterate on the training runs that actually need it. A100 for training, L4 for serving/rollouts/eval, T4 for plumbing. (§3.4)
